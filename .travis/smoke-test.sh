...
.scripts/docker/wait-healthy.sh "${COMPOSE_PROJECT_NAME}_pattern_library_1"
...
[[ "$(curl -sS -H 'Host: unstable--pattern-library.libero.pub' "http://localhost:${HTTP_PORT}" --output /dev/null --write-out '%{http_code}')" == "200" ]]
